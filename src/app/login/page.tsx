import { redirect } from "next/navigation";

import { STEP_PATH, getAccess } from "@/lib/access";
import { signIn } from "@/lib/auth";
import { BrandMark } from "@/components/brand-mark";

/**
 * Sign-in: Google only.
 *
 * There is no password field, no "forgot password", and no username. Not
 * hidden — absent. Every one of those is an attack surface this account does
 * not need to have, and Google already runs the parts that are hard.
 */
export default async function LoginPage({
  searchParams,
}: {
  searchParams: Promise<{ error?: string; next?: string }>;
}) {
  const access = await getAccess();
  if (access.step !== "login") redirect(access.step === "ok" ? "/" : STEP_PATH[access.step]);
  const { error } = await searchParams;

  return (
    <main className="relative z-10 grid min-h-screen place-items-center px-6">
      <div className="glass ring-solana w-full max-w-md rounded-3xl p-10">
        <BrandMark />

        <h1 className="mt-8 text-2xl font-semibold tracking-tight">Sign in</h1>
        <p className="text-[var(--color-ink-secondary)] mt-2 text-sm leading-relaxed">
          Access is limited to allow-listed accounts. After Google you will be
          asked for your PIN — both are required.
        </p>

        {error && (
          <div className="mt-6 rounded-xl border border-[var(--color-loss)]/40 bg-[var(--color-loss)]/10 px-4 py-3 text-sm text-[var(--color-loss)]">
            {error === "AccessDenied"
              ? "That account is not on the allow-list."
              : "Sign-in failed. Please try again."}
          </div>
        )}

        <form
          className="mt-8"
          action={async () => {
            "use server";
            // To /pin; the layout forwards an unapproved device to /device.
            await signIn("google", { redirectTo: "/pin" });
          }}
        >
          <button
            type="submit"
            className="flex w-full items-center justify-center gap-3 rounded-xl bg-[var(--color-ink)] px-5 py-3.5 text-sm font-semibold text-black transition-transform hover:scale-[1.015] active:scale-[0.99]"
          >
            <svg className="h-5 w-5" viewBox="0 0 24 24" aria-hidden>
              <path
                fill="#4285F4"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.76h3.57c2.08-1.92 3.28-4.74 3.28-8.09Z"
              />
              <path
                fill="#34A853"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.76c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23Z"
              />
              <path
                fill="#FBBC05"
                d="M5.84 14.11a6.6 6.6 0 0 1 0-4.22V7.05H2.18a11 11 0 0 0 0 9.9l3.66-2.84Z"
              />
              <path
                fill="#EA4335"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.05l3.66 2.84C6.71 7.31 9.14 5.38 12 5.38Z"
              />
            </svg>
            Continue with Google
          </button>
        </form>

        <p className="text-[var(--color-ink-muted)] mt-8 text-center text-xs">
          CDEXIO manages a live futures account. Sign-in lasts 7 days; approved devices only.
        </p>
      </div>
    </main>
  );
}
