...
export function PositionDetail({
  ...
  // The verdict is fetched here; the chart fetches its own candles
  // (see PositionChart).
  useEffect(() => {
      ...
      {target && (
        ...
        >
          ...
          >
            ...
            {/* The position drawn like TradingView's Long/Short Position tool:
                target and stop zones, each with price, distance and dollars.
                Opens on 4h, the engine's bar. */}
            <div className="mb-5">
              <PositionChart
                symbol={target.symbol}
                side={target.side}
                ...
                exitPrice={target.exitPrice}
                quantity={target.quantity}
                openedAt={target.openedAt}
                closedAt={target.closedAt}
                maxHoldHours={target.maxHoldHours}
                height={560}
              />
            </div>
